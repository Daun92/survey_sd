...
import { Settings2, ChevronDown, ChevronUp, Save, Loader2, X, ImageIcon } from "lucide-react";
import { updateTemplateSettings } from "../../actions";
...
export function SettingsPanel({ templateId, initialSettings, onSettingsChange, onSaved }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await updateTemplateSettings(templateId, settings as Record<string, unknown>);
      onSaved();
    } catch (e) {
        ...
        <div className="px-5 pb-5 border-t border-stone-100 pt-4 space-y-5">
              ...
              <div className="space-y-2">
                {respondentFields.map((field) => (
                  <div key={field.id} className="flex items-center gap-3 rounded-lg border border-stone-200 px-3 py-2">
                    <input type="checkbox" checked={field.enabled} onChange={(e) => updateField(field.id, { enabled: e.target.checked })} className="accent-teal-600" />
                    <input type="text" value={field.label} onChange={(e) => updateField(field.id, { label: e.target.value })} className="flex-1 rounded border border-stone-200 px-2 py-1 text-sm focus:border-teal-500 outline-none" />
                    <label className="flex items-center gap-1.5 text-xs text-stone-500 shrink-0">
                      <input type="checkbox" checked={field.required} onChange={(e) => updateField(field.id, { required: e.target.checked })} className="accent-teal-600" />
                      필수
                    </label>
                  </div>
                ))}
          ...
          <div>
            ...
            <label className="flex items-center gap-2 mt-1.5 text-xs text-stone-500">
              <input type="checkbox" checked={settings.require_consent ?? false} onChange={(e) => update({ require_consent: e.target.checked })} className="accent-teal-600" />
              동의 체크 필수
            </label>
          </div>
